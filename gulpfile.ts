...
gulp.task('replace', () => {
	gulp.src(['./dist/src/routes/swagger.js'])
		.pipe(replace('localhost', '192.168.1.247'))
		.pipe(gulp.dest('./dist/src/routes'));

	gulp.src(['./dist/src/public/docs/index.html'])
		.pipe(replace('localhost', '192.168.1.247'))
		.pipe(gulp.dest('./dist/src/public/docs'));

	gulp.src(['./dist/src/public/index.html'])
	...
	gulp.src(['./dist/src/public/package.json'])
	...
	gulp.src(['./dist/src/package.json'])
		.pipe(
			replace(
				'aumet-admin-app-development',
				'aumet-admin-app-production',
			),
		)
		.pipe(gulp.dest('./dist/src'));
});
...
gulp.task('compile-schema-doc-generator', () => {
	const docGenProj = gulpTs.createProject(
		'schema-doc-generator/tsconfig.json',
	);
	return docGenProj
		.src()
		.pipe(docGenProj())
