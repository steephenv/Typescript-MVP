...
export const getWorkingPeriods = (
  ...
  daysArray.forEach(eachDay => {
    ...
    if (dayInWeek < 6 || dayInWeek > 0 || workingDays.indexOf(dayInWeek) > 0) {
      const periodStart1 = new Date(eachDay.startTime).setHours(
        workingTime.startTime,
        0,
        0,
        0,
      );
      const periodEnd1 = new Date(eachDay.startTime).setHours(
        breakTime.startTime,
        0,
        0,
        0,
      );
      const periodStart2 = new Date(eachDay.startTime).setHours(
        breakTime.endTime,
        0,
        0,
        0,
      );
      const periodEnd2 = new Date(eachDay.startTime).setHours(
        workingTime.endTime,
        0,
        0,
        0,
